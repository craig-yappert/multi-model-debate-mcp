...
import { AIToAIRequest, ConversationEntry } from '../storage/conversation-store';

export interface MCPRequest {
    message: string;
    persona: string;
    vscode_context: VSCodeContext;
...
export class MCPClient {
    ...
    private serverPort: number = 0; // Will be assigned dynamically
    ...
    private resolveMCPServerPath(path: string): string {
    ...
    private async startMCPServer(): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            this.serverProcess = spawn(pythonCmd, [this.serverPath], {
    ...
    private async sendRawMCPRequest(request: any): Promise<any> {
        return new Promise((resolve, reject) => {
            if (!this.serverProcess) {
    ...
    private async sendMCPNotification(notification: any): Promise<void> {
        if (!this.serverProcess) {
    ...
    async contribute(request: MCPRequest): Promise<string> {
        if (!this.isConnected) {
    ...
    async sendAIToAIMessage(request: AIToAIRequest): Promise<string> {
        if (!this.isConnected) {
            const connected = await this.connect();
            if (!connected) {
                throw new Error('Could not connect to MCP server');
        ...
        try {
            ...
            const enhancedRequest: MCPRequest = {
    ...
    private formatConversationContext(context: ConversationEntry[]): string {
        if (!context || context.length === 0) {
            return "No previous conversation context.";
        }

        return context.map(entry => 
            `${entry.persona}: ${entry.message}\nResponse: ${entry.response}`
        ).join('\n\n');
    }

    private createMinimalContext(): VSCodeContext {
    ...
    private async sendMCPRequest(method: string, params: any): Promise<string> {
    ...
    private extractResponseFromOutput(output: string): string {
    ...
    async readDiscussion(): Promise<string[]> {
        if (!this.isConnected) {
            await this.connect();
        }
    ...
    async getHealthStatus(): Promise<{ connected: boolean; serverRunning: boolean; lastError?: string }> {
    ...
    shouldUseMattermostFallback(): boolean {
        const fallbackEnabled = this.config.get<boolean>('enableMattermostFallback', true);
